...
NSString * bidirectional_scroll$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation bidirectional_scrollObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_bidirectional_scroll);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
-(void)injectPatches
{
	// called to inject any bidirectional_scroll patches in JS before a context is loaded... nice for 
...
{
	// create bidirectional_scroll global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_bidirectional_scroll = [[bidirectional_scrollObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_bidirectional_scroll];
	
	NSString *_bidirectional_scrollNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _bidirectional_scrollNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_bidirectional_scroll valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:bidirectional_scroll$ModuleRequireFormat,code];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	if (data!=nil)
	{
		...
		if (![module respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			@throw [NSException exceptionWithName:@"org.bidirectional_scroll.kroll" reason:[NSString stringWithFormat:@"Module \"%@\" failed to leave a valid exports object",path] userInfo:nil];
		}
	...
	@throw [NSException exceptionWithName:@"org.bidirectional_scroll.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
