...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kbidirectional_scrollUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kbidirectional_scrollUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
